...
# 2. Генерация csv файла с тремя случайными числами в каждой строке.
# 100-1000 строк.
# 3. Декоратор, запускающий функцию нахождения корней квадратного
# уравнения с каждой тройкой чисел из csv файла.
# 4. Декоратор, сохраняющий переданные параметры и результаты
...
def gen_csv(filename):
    rows = random.randint(10, 101)
    with open(filename, 'a', newline='', encoding='utf-8') as file_csv:
        writer = csv.writer(file_csv)
        for _ in range(rows):
            writer.writerow([random.randint(1, 50) for _ in range(3)])
